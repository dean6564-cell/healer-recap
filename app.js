...
function recapAvoidableEvidence(d,v){
 const matching=(d.events||[]).filter(e=>e.kind==='damage'&&e.player===d.player&&renderVerifiedActions(v?.spellGuide?.[e.spellId]));
 return {amount:Number.isFinite(d.avoidableDamageTotal)?d.avoidableDamageTotal:matching.reduce((sum,e)=>sum+(Number(e.amount)||0),0),known:Number.isFinite(d.avoidableDamageTotal)||matching.length>0};
}
function renderDeathWindowSummary(d,v){
 const {amount:avoidable}=recapAvoidableEvidence(d,v);
...
function healerDeathContext(r,group){
 ...
 const healerDeath=clock(healer.death.deathTime)||healer.time;
 const later=ordered.filter(x=>x.death!==healer.death&&x.time>healerDeath+.25);
 if(!later.length)return null;
 return {name:healer.death.player,later:new Set(later.map(x=>x.death)),cascade:earlier.length===0};
}
function renderDeathRecaps(r,encounter){
  ...
  ${d.deathSequenceContext?`<p class="recap-note"><strong>Death sequence:</strong> ${esc(d.deathSequenceContext)}</p>`:''}
  ${d.combatResurrectionContext?`<p class="recap-note"><strong>Combat resurrection:</strong> ${esc(d.combatResurrectionContext)}</p>`:''}
...
function setupRecapFilters(root,focusPlayer=''){
 root.querySelectorAll('[data-player-recaps]').forEach(pack=>{
  ...
  if(focusPlayer){const chosen=buttons.find(button=>button.dataset.recapPlayer===focusPlayer);if(chosen)chosen.click()}
 });
...
function setupBossExplorer(r,focusPlayer=''){
  ...
  function paint(){
    ...
    setupRecapFilters(content,focusPlayer);
  }
...
function runHref(id,hash='',member=selectedCharacterRecord()){
 const params=new URLSearchParams({id:String(id)}),character=selectedCharacterParam(member).replace(/^character=/,'');
 if(character)params.set('character',decodeURIComponent(character));
 if(inGuildArea())params.set('group','guild');
 return `run.html?${params}${hash}`;
}
...
function runRow(r){
  ...
  return`<a class="run-row ${art?'run-row-art':''}" ${art?`style="--run-card-art:url(\'${esc(art)}\')"`:''} href="${runHref(r.id)}">
...
function renderCharacterProfileRuns(runs,member){
  ...
  el.innerHTML=records.length?records.map(r=>{
    ...
    return `<${tag} class="profile-run ${tracked?'is-tracked':'is-untracked'}" ${tracked?`href="${runHref(tracked.id,'',member)}"`:''} ${art?`style="--profile-art:url('${esc(art)}')"`:''}>
...
function setupAvoidableView(root){
 ...
 const activate=view=>{
  ...
  metrics.forEach(metric=>{metric.querySelector('b').textContent=view==='full'?(metric.dataset.fullValue||'—'):metric.dataset.deathValue;metric.querySelector('small').textContent=view==='full'?metric.dataset.fullNote:metric.dataset.deathNote;});
