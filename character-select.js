(()=>{
 ...
 const roleIcon=role=>{const icon={Tank:'tank',Healer:'healer',DPS:'dps'}[role];return icon?`<img class="character-role-icon" src="assets/wow-role-${icon}.svg?v=2" alt="${safe(role)}" title="${safe(role)}" width="18" height="18">`:safe(role||'Guild member')};
 let choices=[];
 function render(query=''){
  ...
  grid.innerHTML=visible.map(({member,count,reviewed})=>{const classSlug=String(member.class||'').toLowerCase().replace(/[^a-z]+/g,'-'),realm=String(member.realm||'').replace(/-/g,' ');return `<a class="character-select-card" href="dashboard.html?character=${encodeURIComponent(member.key)}" data-character-key="${safe(member.key)}" style="--class-colour:${colours[member.class]||'#d7ad52'};--class-art:url('assets/class-bg-${safe(classSlug)}.webp')"><img src="${safe(member.avatar||fallback(member))}" data-fallback="${safe(fallback(member))}" alt="" width="74" height="74"><span class="character-select-copy"><strong>${safe(member.name)}</strong><span class="character-select-details">${safe(member.spec||member.class)} <i>·</i> ${roleIcon(member.role)} <i>·</i> ${safe(realm)} <i>·</i> ${safe(String(member.region||'EU').toUpperCase())}${member.level?` <i>·</i> Level ${safe(member.level)}`:''}</span></span><span class="character-run-count"><strong>${count}</strong><small>recorded ${count===1?'run':'runs'}</small>${reviewed?`<em>${reviewed} reviewed</em>`:'<em>Facts available</em>'}</span><span class="character-card-arrow" aria-hidden="true">→</span></a>`}).join('')||'<p class="selector-empty">No recorded guild characters match that search.</p>';
 }
 async function load(){
  try{
   ...
   choices=[...counts].map(([key,count])=>({member:{...roster.get(key),region:roster.get(key).region||guild.guild?.region||'EU'},count,reviewed:reviews.get(key)||0})).sort((a,b)=>b.count-a.count||a.member.name.localeCompare(b.member.name));
   render();
