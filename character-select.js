(()=>{
 ...
 function render(query=''){
  ...
  grid.innerHTML=visible.map(({member,count,reviewed})=>{const characterClass=member.key==='shattered-halls/kazzataur'?'Paladin':member.class,classSlug=String(characterClass||'').toLowerCase().replace(/[^a-z]+/g,'-');return `<a class="character-select-card" href="dashboard.html?character=${encodeURIComponent(member.key)}" data-character-key="${safe(member.key)}" style="--class-colour:${colours[characterClass]||'#d7ad52'};--class-art:url('assets/class-bg-${safe(classSlug)}.webp')"><img src="${safe(member.avatar||fallback({...member,class:characterClass}))}" data-fallback="${safe(fallback({...member,class:characterClass}))}" alt="" width="74" height="74"><span class="character-select-copy"><strong>${safe(member.name)}</strong><span>${safe(member.spec||characterClass)} · ${safe(member.role||'Guild member')}</span><small>${safe(String(member.realm).replace(/-/g,' '))} · EU</small></span><span class="character-run-count"><strong>${count}</strong><small>recorded ${count===1?'run':'runs'}</small>${reviewed?`<em>${reviewed} reviewed</em>`:'<em>Facts available</em>'}</span><span class="character-card-arrow" aria-hidden="true">→</span></a>`}).join('')||'<p class="selector-empty">No recorded guild characters match that search.</p>';
 }
 async function load(){
  try{
